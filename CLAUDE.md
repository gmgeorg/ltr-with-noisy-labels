...
**Design**: Modular with abstract base classes (`NoiseModel`, `Ranker`, `SamplingStrategy`), reproducible via `random_state`, integrates with `choix`

**Modules**: `noise/` (BetaNoiseModel), `ground_truth/`, `comparisons/` (ComparisonData, ComparisonGenerator), `ranking/` (BordaCount, PlackettLuce, RankCentrality), `sampling/` (RandomSampling), `metrics/` (Kendall's tau, top-k), `simulation/` (SimulationRunner)
...
**Sampling**: Inherit from `SamplingStrategy`, implement selection logic
